...
fputs($fp, "<?php\n");
fputs($fp, "// Zugriff ueber \$dbdaten['server'], \$dbdaten['datenbank'] usw.\n");
fputs($fp, "\$dbdaten = array(\n");
fputs($fp, "// DATENBANKSERVER\n");
fputs($fp, "'server' 			=> '$dbhost',\n");
fputs($fp, "// DATENBANK\n");
fputs($fp, "'datenbank' 		=> '$dbname',\n");
fputs($fp, "// DATENBANK USERNAME\n");
fputs($fp, "'user' 				=> '$dbuser',\n");
fputs($fp, "// DATENBANK PASSWORT\n");
fputs($fp, "'passwort' 			=> '$dbpass',\n");
fputs($fp, "// DATENBANK TABELLEN PREFIX\n");
fputs($fp, "'prefix' 			=> '$dbprefix'\n");
fputs($fp, ");\n");
fputs($fp, "?>\n");
fclose($fp);
...
fputs($fp2, $htaccessfileinhalt);
fclose($fp2);
@chmod ('logins/.htaccess', 0666);

$fp3 = fopen('setup/.htaccess','w');
fputs($fp3, $htaccessfileinhalt);
fclose($fp3);
